    ...
    public void fill() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                //MJSV 26112024
                stDoctos = "'" + rs.getString("DOCUMENTO") + "'," + stDoctos;
                ...
                if (valIVA0 == 0) {
                    ...
                    if (rs.getString("IMPUESTO").trim().equals("IVA16")) {
                        if (tcFact == 1) {
                            ...
                            baseTP = baseTP + Double.parseDouble(dec.format((pagado / 1.16)));
                            impuestos = impuestos + Double.parseDouble(dec.format((pagado / 1.16) * .16));
            ...
            if (!stDoctos.isEmpty()) {
                ...
                stDoctos = stDoctos.substring(0, stDoctos.length() - 1);

                ps = BPCSConn.prepareStatement(instance().getDoctosIva00(stDoctos));
                ps.setInt(1, pagoCliente.getTransactionProcess());
                ...
                while (rs.next()) {
                    ...
                    ps = BPCSConn.prepareStatement(instance().getImpuestosIVA00TDR());
                    ps.setInt(1, pagoCliente.getTransactionProcess());
                    ps.setInt(2, rs.getInt("FOLIOPAGO"));

                    ResultSet rs1 = ps.executeQuery();

                    while (rs1.next()) {
                        iva00 = iva00 + rs1.getDouble("IMPAGO");
                    }

                }
            } else {
                //CASO DEL CDP 5012722 MJSV 30-01-2025

                ps = BPCSConn.prepareStatement(instance().getImpuestosIVA00TDR1());
                ps.setInt(1, pagoCliente.getTransactionProcess());
