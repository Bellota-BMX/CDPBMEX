...
//                + "ORDER BY FOLIO";
        ...
        sql = "SELECT S.FECHA,\n"
                ...
                + "       C.CCUEN AS CUENTA,\n"
                + "       C.CRFCB AS RFC,\n"
                + "       (SELECT MAX(CTEXT) FROM MP61BPCUSF.COMPBMXD C  WHERE C.CTRAN = X.CTRAN AND C.CFEPA >= ? AND C.CFEPA <= ? AND (CTEXT LIKE '%01%' OR CTEXT LIKE '%03%' OR CTEXT LIKE '%02%') GROUP BY CTRAN, CTEXT ORDER BY CTEXT DESC LIMIT 1) AS MENSAJE, \n"
                + "       C.CESTD AS TXT "
...
//                + "ORDER BY X.CFOLIO, CDOCU";
        ...
        //MJSV 17052024
//        sql = "  SELECT H3TRNP AS CTRAN,\n"
//                + "     H3DOCN AS FACTURA,\n"
//                + "     CO.CPARC AS PARCIALIDAD,\n"
//                + "     S.SITOT AS SALDO_ANTERIOR,\n"
//                + "     SUM(R.H3TAMT) AS MONTO_PAGO,  \n"
//                + "     (S.SITOT-SUM(R.H3TAMT)) AS NUEVO_SALDO, \n"
//                + "     CASE WHEN MAX(S.SICURR) = 'PSO' THEN 'MXN' WHEN MAX(S.SICURR) <> 'PSO' THEN MAX(S.SICURR) END AS MONEDA, \n"
//                + "     CBANK AS BANCO, \n"
//                + "     CCUEN AS CUENTA, \n"
//                + "     CRFCB AS RFC, \n"
//                + "     CTEXT AS MENSAJE, \n"
//                + "     S.IHDPFX AS PREFIJO\n"
//                + "  FROM MP61CARF.RDH3 R\n"
...
//                + "  WHERE CFEPA >= ? AND CFEPA <= ? \n"
//                + "        AND H3TRNP = ? \n"
//                + "        AND SUBSTRING(H3DREF,1,2) IN ('SP','FF') \n"
//                + "  GROUP BY H3TRNP, H3DOCN, CO.CPARC, S.SITOT,CBANK, CCUEN, CRFCB, CTEXT, S.IHDPFX \n"
//                + "  ORDER BY H3TRNP, H3DOCN, CO.CPARC";
        //MJSV 31052024
        sql = "SELECT R.H3TRNP AS CTRAN,\n"
                + "   R.H3DOCN AS FACTURA,\n"
                + "   CO.CPARC AS PARCIALIDAD,\n"
                + "   CO.CIMPO AS SALDO_ANTERIOR,\n"
                + "   SUM(R.H3TAMT) AS MONTO_PAGO,  \n"
                + "   CO.CSALD AS NUEVO_SALDO, \n"
                + "   CASE WHEN MAX(R.H3CURR) = 'PSO' THEN 'MXN' WHEN MAX(R.H3CURR) <> 'PSO' THEN MAX(R.H3CURR) END AS MONEDA, \n"
                + "   CO.CBANK AS BANCO, \n"
                + "   CO.CCUEN AS CUENTA, \n"
                + "   CO.CRFCB AS RFC, \n"
                + "   CO.CTEXT AS MENSAJE, \n"
                + "   R.H3DPX AS PREFIJO\n"
                + "   FROM MP61CARF.RDH3 R\n"
                + "   LEFT JOIN MP61BPCUSF.COMPBMXD AS CO ON H3DOCN = CO.CDOCU AND CO.CTRAN = R.H3TRNP\n"
                + "   LEFT JOIN MP61BPCSF.ZCC AS Z ON CCTABL = 'CDPCFD40' AND CCCODE = 'FOLIO' \n"
                + "   WHERE CFEPA >= ? AND CFEPA <= ? \n"
                + "   AND H3TRNP = ? \n"
                + "   AND SUBSTRING(H3DREF,1,2) IN ('SP','FF') \n"
                + "   GROUP BY R.H3TRNP, R.H3DOCN, CO.CPARC,CO.CIMPO, CO.CSALD, CO.CBANK, CO.CCUEN, CO.CRFCB, CO.CTEXT, R.H3DPX \n"
                + "   ORDER BY R.H3TRNP, R.H3DOCN, CO.CPARC";
    ...
    public String getFTP() { //MJSV 10052024

        String sql;

        sql = "SELECT CCDESC AS IP, CCNOT1 AS USUARIO, CCNOT2 AS PASS FROM MP61BPCSF.ZCC WHERE CCTABL = 'SATPRMFE' AND CCCODE = 'ACCESOMEX'";
...
//                + "WHERE R.H3DREF IN ('SP','FF') AND  I.TXTRCD LIKE ? AND R.H3TRNP = ? GROUP BY I.TXINVN";
        sql = "SELECT MAX(I.TXCUST) AS CLIENTE, "
                ...
                + "MAX(I.TXTXBA) AS BASETDR,"
                + "/*SUM(DISTINCT(R.H3TAMT)) AS BASETP,*/ "
                + "(SELECT SUM(RX.H3TAMT) FROM MP61CARF.RDH3 AS RX WHERE RX.H3TRNP = R.H3TRNP AND RX.H3DOCN = I.TXINVN AND RX.H3DREF IN ('SP', 'FF')) AS BASETP,"
                + "MAX(I.TXTAXT) AS IMPORTETDR, "
                + "SUM(DISTINCT(S.SITOT)) AS VFACTURA, "
                + "MAX(Z.CCDESC) AS IMPUESTOTDR, "
                + "MAX(Z.CCSDSC) AS TIPOFACTORTDR, "
                ...
                + "INNER JOIN MP61BPCSF.SIH AS S ON R.H3DPX=S.IHDPFX AND R.H3DYR=S.IHDYR AND R.H3DTP=S.IHDTYP AND  R.H3DOCN=S.IHDOCN AND I.TXINVN =  R.H3DOCN "
                + "INNER JOIN MP61BPCSF.ZCC AS Z ON I.TXTRCD = Z.CCCODE AND z.CCTABL = 'SATIMPCO' "
                + "WHERE R.H3DREF IN ('SP','FF') AND  I.TXTRCD LIKE ? AND R.H3TRNP = ? "
                + "GROUP BY I.TXINVN, R.H3TRNP ORDER BY I.TXINVN ASC";
...
//                + "WHERE R.H3TRNP = ? AND R.H3DREF IN ('SP','FF') GROUP BY R.H3TIME, R.H3PAYC , R.H3PAYC, R.H3CURR, R.H3CNVF,R.H3TRNP, B.B7RFCB ,  B.B7BANK,  B.B7ACCT, R.H3DREF, R.H3TDTE,Z.CCCODE,BANK.BKACCT,BANK.BKDATN,R.H3DREF,R.H3CUST";
        ...
        sql = "SELECT \r\n"
                ...
                + " (SELECT DISTINCT(CBANK) FROM MP61BPCUSF.COMPBMXD WHERE CTRAN = R.H3TRNP LIMIT 1) AS BANCO,\n"
                + " (SELECT DISTINCT(CCUEN) FROM MP61BPCUSF.COMPBMXD WHERE CTRAN = R.H3TRNP LIMIT 1) AS CUENTA,\n"
                + " (SELECT DISTINCT(CRFCB) FROM MP61BPCUSF.COMPBMXD WHERE CTRAN = R.H3TRNP LIMIT 1) AS RFC "
                + "FROM MP61CARF.RDH3 AS R \r\n"
...
//                + "LEFT JOIN MP61BPCUSF.COMPBMXD AS CO ON H3DOCN=CO.CDOCU "
//                + "LEFT JOIN MP61BPCSF.ZCC AS Z ON CCTABL = 'CDPCFD40' AND CCCODE = 'FOLIO'"
//                + "WHERE H3TRNP = ? AND SUBSTRING(H3DREF,1,2) IN ('SP','FF') "
//                + "GROUP BY S.IHDOCN,R.H3CUST,CO.CBANK,CO.CCUEN,CO.CRFCB,R.H3DREF";
        //MJSV 31052024
//        sql = " SELECT MAX(S.IHDPFX) AS SERIEPAGO, \n"
//                + "                       S.IHDOCN      AS FOLIOPAGO, \n"
//                + "                       MAX(Z.CCDESC) AS FOLIO,\n"
...
//                + "                       MAX(R.H3TDTE) AS FECHAPAGO,\n"
//                + "                       (SELECT COUNT(*) FROM MP61BPCUSF.BILB07 WHERE B7CUST = R.H3CUST AND B7ID = 'B7') AS NUMCUEN, \n"
//                + "                       (SELECT DISTINCT(C.CCUEN) FROM MP61BPCUSF.COMPBMXD C WHERE C.CTRAN = R.H3TRNP LIMIT 1) AS CINSER,\n"
//                + "                       (SELECT BLUUID AS UUID FROM MP61BPCUSF.BILB05 WHERE BLDOCU = S.IHDOCN AND BLPREF =  MAX(S.IHDPFX)), \n"
//                + "                       '02' AS OBJETOIMP, \n"
//                + "                       '01' AS EQUIVALENCIADR,\n"
//                + "                       R.H3TRNP, \n"
//                + "                       (SELECT DISTINCT(C.CBANK) FROM MP61BPCUSF.COMPBMXD C WHERE C.CTRAN = R.H3TRNP LIMIT 1) AS CBANK, \n"
//                + "                       (SELECT DISTINCT(C.CRFCB) FROM MP61BPCUSF.COMPBMXD C WHERE C.CTRAN = R.H3TRNP LIMIT 1) AS CRFCB,  \n"
//                + "                       MAX(R.H3CNVF) AS TCAMBIO, \n"
//                + "                       R.H3DREF AS TIP \n"
//                + "                  FROM MP61CARF.RDH3 AS R \n"
//                + "                  INNER JOIN MP61BPCSF.SIH AS S ON R.H3DPX = IHDPFX AND R.H3DYR = IHDYR AND R.H3DTP = IHDTYP AND R.H3DOCN = IHDOCN \n"
//                + "                  LEFT JOIN MP61BPCUSF.COMPBMXD AS CO ON H3DOCN = CO.CDOCU \n"
//                + "                  LEFT JOIN MP61BPCSF.ZCC AS Z ON CCTABL = 'CDPCFD40' AND CCCODE = 'FOLIO'\n"
//                + "                  WHERE R.H3TRNP = ? AND SUBSTRING(R.H3DREF,1,2) IN ('SP','FF') \n"
//                + "                  GROUP BY S.IHDOCN,R.H3CUST,R.H3DREF,R.H3TRNP";
        ...
        sql = "SELECT MAX(S.IHDPFX) AS SERIEPAGO, \n"
                ...
                + "       CASE WHEN MAX(S.SICURR) = 'PSO' THEN 'MXN' WHEN  MAX(S.SICURR) <> 'PSO' THEN MAX(S.SICURR) END AS MONEDADR, \n"
                + "       /*SUM(DISTINCT(R.H3TAMT)) AS IMPAGO,*/ \n"
                + "       (SELECT SUM(RX.H3TAMT) FROM MP61CARF.RDH3 AS RX WHERE RX.H3TRNP = R.H3TRNP AND H3DOCN = S.IHDOCN AND RX.H3DREF IN ('SP', 'FF') GROUP BY RX.H3CUST, RX.H3CNVF, RX.H3TRNP) AS IMPAGO,\n"
                + "       MAX(R.H3CUST) AS CLIENTE, \n"
                + "       SUM(DISTINCT(S.SITOT)) AS VFACTURA, \n"
                + "       SUM(DISTINCT(S.SITAX)) AS SUMAVFACT, \n"
                + "       MAX(R.H3TSEQ) AS SECUENCIA, \n"
                + "       MAX(CO.CPARC) AS PARCIALIDAD2, \n"
                + "       /*(SELECT CPARC FROM MP61BPCUSF.COMPBMXD AS CO WHERE CDOCU = S.IHDOCN AND CTRAN  = R.H3TRNP LIMIT 1) AS PARCIALIDAD2,*/"
                + "       MAX(R.H3TDTE) AS FECHAPAGO,\n"
                + "       (SELECT COUNT(*) FROM MP61BPCUSF.BILB07 WHERE B7CUST = R.H3CUST AND B7ID = 'B7') AS NUMCUEN,\n"
                + "       (SELECT DISTINCT(C.CCUEN) FROM MP61BPCUSF.COMPBMXD C WHERE C.CTRAN = R.H3TRNP LIMIT 1) AS CINSER,\n"
                + "       (SELECT BLUUID AS UUID FROM MP61BPCUSF.BILB05 WHERE BLDOCU = S.IHDOCN AND BLPREF =  MAX(S.IHDPFX)), \n"
    ...
    public String getImpuestosBB() {
        ...
        sql = "SELECT DISTINCT TXTRCD FROM MP61BPCSF.RTX AS I \n"
                + "INNER JOIN MP61CARF.RDH3 AS R ON R.H3DOCN = I.TXINVN AND R.H3CUST = I.TXCUST \n"
                + "WHERE TXTRCD LIKE ? AND R.H3TRNP = ? AND R.H3DREF IN ('SP','FF') AND I.TXINVN = ?";
...
//                + "WHERE R.H3DREF IN ('SP','FF') AND  I.TXTRCD LIKE ? AND R.H3TRNP = ? AND I.TXINVN = ?  GROUP BY I.TXINVN";
        ...
        sql = "SELECT MAX(I.TXCUST) AS CLIENTE, "
                ...
                + "INNER JOIN MP61BPCSF.SIH AS S ON R.H3DPX=S.IHDPFX AND R.H3DYR=S.IHDYR AND R.H3DTP=S.IHDTYP AND  R.H3DOCN=S.IHDOCN AND I.TXINVN =  R.H3DOCN "
                + "INNER JOIN MP61BPCSF.ZCC AS Z ON I.TXTRCD = Z.CCCODE AND Z.CCTABL = 'SATIMPCO' "
                + "WHERE R.H3DREF IN ('SP','FF') AND  I.TXTRCD LIKE ? AND R.H3TRNP = ? AND I.TXINVN = ?  "
                + "GROUP BY I.TXINVN, R.H3TRNP";
    ...
    public String getImpuestosIVA00() {
        ...
        sql = " SELECT SUM(ILREV) AS SUBTOTAL, SUM(ILTA01) AS IVA, ILTR01 AS IVA \n"
                + " FROM MP61BPCSF.SIL \n"
                + " WHERE ILINVN = ? AND (ILTR01 = 'IVA00' OR ILTR01 = '') AND ILDPFX = 'FA' \n"
                + " GROUP BY ILTR01";
...
//                + "MAX(I.TXTRCD) AS IMPUESTO "
//                + "FROM MP61CARF.RDH3 AS R \n"
//                + "INNER JOIN MP61BPCSF.RTX I ON R.H3DOCN = I.TXINVN AND R.H3CUST = I.TXCUST  \n"
//                + "INNER JOIN MP61BPCSF.SIH AS S ON H3DPX=IHDPFX AND H3DYR=IHDYR AND H3DTP=IHDTYP AND  H3DOCN=IHDOCN \n"
//                + "LEFT JOIN MP61BPCUSF.COMPBMXD AS CO ON H3DOCN=CO.CDOCU\n"
//                + "LEFT JOIN MP61BPCSF.ZCC AS Z ON CCTABL = 'CDPCFD40' AND CCCODE = 'FOLIO'"
//                + "WHERE H3TRNP = ? AND SUBSTRING(H3DREF,1,2) IN ('SP','FF') AND CO.CTRAN = ? AND S.IHDOCN = ? GROUP BY S.IHDOCN,R.H3CUST";
        ...
        sql = "SELECT MAX(S.IHDPFX) AS SERIEPAGO, \n"
                ...
                + "       MAX(Z.CCDESC) AS FOLIO,\n"
                + "       CASE WHEN MAX(S.SICURR) = 'PSO' THEN 'MXN' WHEN  MAX(S.SICURR) <> 'PSO' THEN MAX(S.SICURR) END AS MONEDADR, \n"
                + "       /*MAX(R.H3TAMT) AS IMPAGO,*/\n"
                + "      (SELECT SUM(RX.H3TAMT) FROM MP61CARF.RDH3 AS RX WHERE RX.H3TRNP = R.H3TRNP AND H3DOCN = S.IHDOCN AND RX.H3DREF IN ('SP', 'FF') GROUP BY RX.H3CUST, RX.H3CNVF, RX.H3TRNP) AS IMPAGO, \n"
                + "       MAX(R.H3CUST) AS CLIENTE, \n"
                + "       SUM(S.SITOT) AS VFACTURA, \n"
                + "       SUM(S.SITAX) AS SUMAVFACT,  \n"
                + "       MAX(R.H3TSEQ) AS SECUENCIA, \n"
                + "       MAX(CO.CPARC) AS PARCIALIDAD2,\n"
                + "       MAX(R.H3TDTE) AS FECHAPAGO,\n"
                + "       (SELECT COUNT(*) FROM MP61BPCUSF.BILB07 WHERE B7CUST = R.H3CUST AND B7ID = 'B7') AS NUMCUEN, \n"
                ...
                + "  LEFT JOIN MP61BPCUSF.COMPBMXD AS CO ON H3DOCN=CO.CDOCU\n"
                + "  LEFT JOIN MP61BPCSF.ZCC AS Z ON CCTABL = 'CDPCFD40' AND CCCODE = 'FOLIO'\n"
                + "  WHERE H3TRNP = ? AND SUBSTRING(H3DREF,1,2) IN ('SP','FF') AND CO.CTRAN = ? AND S.IHDOCN = ? \n"
                + "  GROUP BY R.H3TRNP, S.IHDOCN, R.H3CUST";
    ...
    public String getImpuestosAA() {

        String sql;

        sql = "SELECT DISTINCT TXTRCD "
                + "FROM MP61BPCSF.RTX AS I\n"
                + "  INNER JOIN MP61CARF.RDH3 AS R ON R.H3DOCN = I.TXINVN AND R.H3CUST = I.TXCUST\n"
                + "  WHERE R.H3TRNP = ? AND TXTRCD LIKE ? AND R.H3DREF IN ('SP','FF')";

        return sql;

    }

    public String getImpuestosIVA00TDR() {
        /*MJSV 06062024*/
        ...
        sql = "SELECT S.IHDOCN AS FOLIOPAGO, MAX(R.H3TAMT) AS IMPAGO, MAX(Z.CCNOT1) AS TASAOCUOTATDR, MAX(Z.CCDESC) AS IMPUESTOTDR, MAX(Z.CCSDSC) AS TIPOFACTORTDR\n "
                + "  FROM MP61CARF.RDH3 AS R\n "
                + "  INNER JOIN MP61BPCSF.SIH AS S ON R.H3DPX=S.IHDPFX AND R.H3DYR=S.IHDYR AND R.H3DTP=S.IHDTYP AND  R.H3DOCN=S.IHDOCN\n "
                + "  INNER JOIN MP61BPCSF.ZCC AS Z ON  Z.CCCODE = 'IVA00' AND Z.CCTABL = 'SATIMPCO'\n "
                + "WHERE H3TRNP = ? AND SUBSTRING(H3DREF,1,2) IN ('SP','FF') AND S.IHDOCN = ? GROUP BY S.IHDOCN ";
    ...
    public String getImpuestosIVA00TDR1() {
        /*MJSV 06062024*/
        ...
        sql = "SELECT S.IHDOCN AS FOLIOPAGO, MAX(R.H3TAMT) AS IMPAGO, MAX(Z.CCNOT1) AS TASAOCUOTATDR, MAX(Z.CCDESC) AS IMPUESTOTDR, MAX(Z.CCSDSC) AS TIPOFACTORTDR\n "
                ...
                + "  INNER JOIN MP61BPCSF.SIH AS S ON R.H3DPX=S.IHDPFX AND R.H3DYR=S.IHDYR AND R.H3DTP=S.IHDTYP AND  R.H3DOCN=S.IHDOCN\n "
                + "  INNER JOIN MP61BPCSF.ZCC AS Z ON  Z.CCCODE = 'IVA00' AND Z.CCTABL = 'SATIMPCO'\n "
                + "WHERE H3TRNP = ? AND SUBSTRING(H3DREF,1,2) IN ('SP','FF') GROUP BY S.IHDOCN ";
    ...
    public String getParcialidad() {
        ...
        sql = "SELECT CO.CPARC FROM MP61BPCUSF.COMPBMXD CO WHERE CO.CDOCU = ? AND CO.CTRAN = ? LIMIT 1";
        
        return sql;
    }
    
    public String getDoctosIva00(String stDoctos){
        /*MJSV 26112024*/
        
        String sql;
        
        sql = "SELECT S.IHDOCN AS FOLIOPAGO \n" +
            "  FROM MP61CARF.RDH3 AS R \n" +
            "  INNER JOIN MP61BPCSF.SIH AS S ON R.H3DPX = IHDPFX AND R.H3DYR = IHDYR AND R.H3DTP = IHDTYP AND R.H3DOCN = IHDOCN \n" +
            "  LEFT JOIN MP61BPCUSF.COMPBMXD AS CO ON H3DOCN = CO.CDOCU \n" +
            "  LEFT JOIN MP61BPCSF.ZCC AS Z ON CCTABL = 'CDPCFD40' AND CCCODE = 'FOLIO'\n" +
            "WHERE R.H3TRNP = ? AND SUBSTRING(R.H3DREF,1,2) IN ('SP','FF') AND CHAR(S.IHDOCN) NOT IN("+stDoctos+")\n" +
            "GROUP BY S.IHDOCN,R.H3CUST,R.H3DREF,R.H3TRNP\n" +
            "ORDER BY FOLIOPAGO ASC";
