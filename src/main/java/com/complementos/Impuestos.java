    ...
    public static boolean Iva00(PagoCliente pagoCliente, int factura) {
        ...
        try {
            ...
            if (rs.next()) {
                ...
                while (rs.next()) {
                    if(!rs.getString("IVA").trim().equals("IVA16") && rs.getFloat("SUBTOTAL")>0){
                        esiva0 = true;
                    }
                }
            }
        ...
        try {
            ...
            if (rs.next()) {
                ...
                while (rs.next()) {
                    ...
                    pagIVA = Double.parseDouble(dec.format(pagIVA));
                    
                    if (rs.getDouble("BASETDR") == rs.getDouble("VFACTURA")) {
                        esIVA00 = false;
                    } else {
                        if (( Double.parseDouble(dec.format(rs.getDouble("IMPORTETDR") + rs.getDouble("BASETDR"))) != rs.getDouble("VFACTURA"))) {
                            VFACTURA = Double.parseDouble(dec.format( Double.parseDouble(dec.format(rs.getDouble("VFACTURA"))) - (Double.parseDouble(dec.format(rs.getDouble("IMPORTETDR"))) + Double.parseDouble((dec.format(rs.getDouble("BASETDR"))))) ));
                        }
                    }
                }
                ...
                while (rs.next()) {
                    pag = Double.parseDouble(dec.format(pag + rs.getDouble("MONTO")));
                }
